...
- **對戰動漫場景**（v122）：神木/苗圃/岩石/花叢/裝飾樹全改 `assets/field/*.png` 動漫插畫(圖檔優先+canvas fallback；裝飾樹參與 y 排序遮擋)、斜射天光光束(夜間轉月光/暴雨關閉)、飄落花瓣氛圍粒子(上限 22 回收)、賽璐璐飽和度增益(`saturation` 合成)。角色維持寫實立繪不變。**植物類素材產圖須用洋紅背景 #ff00ff**(綠幕會吃掉綠葉)，keyer 用 `scratchpad/keyer-magenta.mjs`。
...
- **PWA 版本**：`sw.js` CACHE 目前 `shoutu-v122`。
