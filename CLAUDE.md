...
- **獎勵揭曉＋對戰報告**（v125）：勝利結算獎勵改逐項彈出徽章(每項一聲叮、大獎 levelup 音)＋彩帶飄落(16 片自動回收)；結算加「📋 對戰報告」——每位守護者的驅逐/造成傷害/承受傷害(hurt/onDeath 歸屬統計，投射物/必殺帶 own)、最佳表現標 ⭐、勝敗都顯示。測試 hook：`window.__dbgEnd(win)` 直接觸發結算。(v126) 結算圖示全改自繪 inline SVG(`svgIco`/ICO 表)——不用手機內建 emoji，徽章/報告/彩帶/標題按鈕皆換。
- **PWA 版本**：`sw.js` CACHE 目前 `shoutu-v126`。
