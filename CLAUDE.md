...
- **對戰動漫場景**（v122）：神木/苗圃/岩石/花叢/裝飾樹全改 `assets/field/*.png` 動漫插畫(圖檔優先+canvas fallback；裝飾樹參與 y 排序遮擋)、斜射天光光束(夜間轉月光/暴雨關閉)、飄落花瓣氛圍粒子(上限 22 回收)、賽璐璐飽和度增益(`saturation` 合成)。角色維持寫實立繪不變。**植物類素材產圖須用洋紅背景 #ff00ff**(綠幕會吃掉綠葉)，keyer 用 `scratchpad/keyer-magenta.mjs`。(v123) 立繪跑動時下半身切前/後半繞髖反相擺動(剪紙木偶步態)＋身體微搖；戰場服裝改錨定立繪頭頂/身體中心並跟隨彈跳/突進(`u._bbH/_bobY/_lgX`)，補上角色專屬服裝(x_*)戰場通用星光環。
...
- **PWA 版本**：`sw.js` CACHE 目前 `shoutu-v128`。
