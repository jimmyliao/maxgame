...
- **首領挑戰難度晉級**：對戰保留模式選擇；一般/限時/防衛只有三人/五人，**單人只在首領挑戰**(單人/三人/五人)，打贏自動晉級（見習→初階→中階→高階→大師），首領隨人數＋難度等級變強；好友房間也可選首領挑戰讓多位真人一起打大首領（快照同步 `isBoss`＋`duel` 狀態，guest 端也顯示雙方血條）。預設解鎖 5 隻守護者(石虎/黑熊/爺蟬/勾蜓/梅花鹿)，足夠 5 位真人各選不同。
...
- **對戰動漫場景**（v122）：神木/苗圃/岩石/花叢/裝飾樹全改 `assets/field/*.png` 動漫插畫(圖檔優先+canvas fallback；裝飾樹參與 y 排序遮擋)、斜射天光光束(夜間轉月光/暴雨關閉)、飄落花瓣氛圍粒子(上限 22 回收)、賽璐璐飽和度增益(`saturation` 合成)。角色維持寫實立繪不變。**植物類素材產圖須用洋紅背景 #ff00ff**(綠幕會吃掉綠葉)，keyer 用 `scratchpad/keyer-magenta.mjs`。(v129) 立繪跑動改「一腳一腳」交替步態：下半身切左右兩腳反相位繞髖擺動＋往前跨的腳抬起(另一腳著地)＋身體微搖；戰場服裝改錨定立繪頭頂/身體中心並跟隨彈跳/突進(`u._bbH/_bobY/_lgX`)，補上角色專屬服裝(x_*)戰場通用星光環。
...
- **獎勵揭曉＋對戰報告**（v125）：勝利結算獎勵改逐項彈出徽章(每項一聲叮、大獎 levelup 音)＋彩帶飄落(16 片自動回收)；結算加「📋 對戰報告」——每位守護者的驅逐/造成傷害/承受傷害(hurt/onDeath 歸屬統計，投射物/必殺帶 own)、最佳表現標 ⭐、勝敗都顯示。測試 hook：`window.__dbgEnd(win)` 直接觸發結算。(v126) 結算圖示全改自繪 inline SVG(`svgIco`/ICO 表)——不用手機內建 emoji，徽章/報告/彩帶/標題按鈕皆換。(v127) 結算頁改可上下捲動(手機一定看得到回大廳)；戰場只顯示實體頭飾(王冠/蝴蝶結)、發光光環類服裝戰場不畫(玩家定案)；**全 13 隻守護者 360° 四視角完成**(assets/views/ 52 張、洋紅產圖管線)。
- **對戰效能優化**（v128）：修「移動卡卡」——① 移除全螢幕 `saturation` 混合(GPU 回讀殺手) ② 靜態地景(森林牆/草/花/溪流/岩石)烘焙到離屏世界畫布每幀只 blit 一次(原本每幀畫 150 草+350 花瓣弧無視野裁切) ③ 立繪 512px 預縮小到 280px 小圖快取(`spScaled`)每幀貼小圖 ④ 方向光/暗角漸層快取(resize 才重建)、光束改單色 ⑤ 重用實體排序陣列降 GC 停頓。實測中階手機模擬 FPS 明顯回升，畫面視覺無明顯差異。
- **PWA 版本**：`sw.js` CACHE 目前 `shoutu-v129`。
